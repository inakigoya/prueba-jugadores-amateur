  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <a href={src || '#'} target="_blank" rel="noreferrer" style={{
        ...
        background: src ? 'var(--blue-bg)' : 'var(--bg2)',
    ...
    <div className="card" style={{ marginBottom: 10, padding: 0, overflow: 'hidden', border: open ? '0.5px solid var(--border2)' : undefined }}>
      ...
      {open && (
        <div style={{ borderTop: '0.5px solid var(--border)', padding: 16 }}>
          ...
          {sol.estado === 'aprobado' && (
            <div style={{ padding: '8px 12px', borderRadius: 'var(--radius)', background: 'var(--green-bg)', color: 'var(--green-text)', fontSize: 13 }}>
              Aprobado y enviado al sistema
            </div>
          )}
          {sol.estado === 'rechazado' && (
            <div style={{ padding: '8px 12px', borderRadius: 'var(--radius)', background: 'var(--red-bg)', color: 'var(--red-text)', fontSize: 13 }}>
...
function EquiposModal({ onClose }) {
  ...
  async function eliminar(id) {
    await fetch(`${API}/equipos/${id}`, { method: 'DELETE' }); load()
  }

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', alignItems: 'flex-start', justifyContent: 'center', padding: '2rem 1rem', zIndex: 100 }}
      onClick={onClose}>
    ...
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '2rem 1rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', flexWrap: 'wrap', gap: 12 }}>
        <div>
          ...
          <p style={{ color: 'var(--text2)', fontSize: 13 }}>Revisá y aprobá las solicitudes antes de cargarlas al sistema</p>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 10, marginBottom: '1.5rem' }}>
        {[{ label: 'Pendientes', val: stats.pendiente, color: 'var(--amber-text)' },
          ...
          { label: 'Rechazados', val: stats.rechazado, color: 'var(--red-text)' }].map(m => (
